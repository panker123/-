...
from train_utils import train_and_evaluate,validate_model
...
def run(args):
    ...
    for epoch in range(args.epochs):
        ...
        for i in range(args.clients_number):
            ...
            #单个用户进行训练
            local_model=train_and_evaluate(args, args.run, tokenizer, train_data[i], compute_metrics,w_glob)
            w_locals.append(local_model)
            del local_model
        ...
        #验证
        validate_model(args, args.run, tokenizer, tokenized_datasets['valid'], compute_metrics,w_glob)
        del w_locals
        torch.cuda.empty_cache()
        #每轮结束后减少对应比例的学习率
        args.lr = args.lr - (args.lr / args.epochs)
